...
function getRectangleString(width, height) {
    let x = '┌' + '─'.repeat(width-2) + '┐' + '\n';
    for (let i=0; i<height-2; i++) {
        x = x.concat('│' + ' '.repeat(width-2) + '│' + '\n');
    }
    x = x.concat('└' + '─'.repeat(width-2) + '┘' + '\n');
    return x;
}
...
function encodeToRot13(str) {
    let alphabetOne = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let alphabetTwo = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let strTwo = '';
    for (let i=0; i<str.length; i++) {
        if (alphabetOne.indexOf(str[i]) == -1) {
            strTwo = strTwo.concat(str[i]);
        } else {
            strTwo = strTwo.concat(alphabetTwo[alphabetOne.indexOf(str[i])]);
        }
    }
    return strTwo;
}
